...
Defined as: $\mathcal{U}(t-a)\begin{cases}
    0 & 0 \leq t < a
    \\ 1 & t \geq a
\end{cases}$, jump occurs at $t$ value where the argument equals zero
\\$\mathcal{U}(t-a)$: \begin{tikzpicture}[>=stealth,scale=1.0, baseline={(current bounding box.center)}]
    \draw[->] (-0.5,0) -- (4.5,0) node[right] {\(t\)};\draw[->] (0,-0.5) -- (0,1.5) node[above] {\(y\)};\draw[dashed] (2,-0.5) -- (2,1.5);\draw[thick,red] (0,0) -- (2,0);\draw[thick,red] (2,1) -- (4,1);\draw[fill=white,draw=red,thick] (2,0) circle (2pt);\draw[fill=red,draw=red] (2,1) circle (2pt);\node[below] at (2,0) {\(a\)};\node[left]  at (0,1) {\(1\)};\node[right] at (4,1) {\(\mathcal{U}(t-a)\)};
\end{tikzpicture}
\\$k\mathcal{U}(t-a)$: \begin{tikzpicture}[>=stealth,scale=1.0, baseline={(current bounding box.center)}]
    \draw[->] (-0.5,0) -- (4.5,0) node[right] {\(t\)};\draw[->] (0,-0.5) -- (0,2.5) node[above] {\(y\)};\draw[dashed] (2,-0.5) -- (2,2.5);\draw[thick,orange] (0,0) -- (2,0);\draw[thick,orange] (2,2) -- (4,2);\draw[fill=white,draw=orange,thick] (2,0) circle (2pt);\draw[fill=orange,draw=orange] (2,2) circle (2pt); \node[below] at (2,0) {\(a\)}; \node[left]  at (0,2) {\(k\)}; \node[right] at (4,2) {\(k\,\mathcal{U}(t-a)\)};
\end{tikzpicture}
\subsubsection{Differentiation: Derivatives of Transforms}
$\mathcal{L}\left\{t^nf(t)\right\}=(-1)^n\frac{d^n}{ds^n}\mathcal{L}\left\{f(t)\right\}=(-1)^n\frac{d^n}{ds^n}F(s)$
\subsubsection{Examples}
$\mathcal{L}\left\{te^{at}\right\}=\frac{d}{ds}\mathcal{L}\left\{e^{at}\right\}=-\frac{d}{ds}\left[\frac{1}{s-a}\right]=\frac{1}{(s-a)^2}$
\\$\mathcal{L}\left\{t^2 \cos kt\right\}= \frac{d^2}{ds^2}\mathcal{L}\left\{\cos kt\right\}=-\frac{d}{ds}\left[\frac{s^2-k^2}{(s^2+k^2)^3}\right]=  \frac{2s(3^2-3k^2)}{(s^2+k^2)^3}$
\\$\mathcal{L}\left\{te^{at}\sin kt \right\} =-\frac{d}{ds}\left[\mathcal{L}\left\{e^{at} \sin kt\right\}\right]=
-\frac{d}{ds}\left[\mathcal{L}\left\{\sin kt\right\}\right]_{s\to (s-a)}=-\frac{d}{ds}\left[k(s^2+k^2)^{-1}\right]_{s\to (s-a)}\\ = -k (-1)(s^2+k^2)^{-2}\cdot 2s |_{s\to (s-a)}=\frac{2ks}{(s^2+k^2)^2|_{s\to (s-a)}}=$\boxed{\frac{2k(s-a)}{\left[(s-a)^2+k^2\right]^2}}
...
\\$\mathcal{L}\left\{\frac{d}{dt}\left[t\cos kt\right]\right\}=\mathcal{L}\left\{f'(t)\right\}=s\mathcal{L}\left\{f(t)\right\}-\cancelto{0}{f(0)}=s\mathcal{L}\left\{t\cos kt\right\}=$\boxed{\frac{s(s^2-k^2)}{(s^2+k^2)^2}}
...
\\$2(f*g)(t)=\sin t-\cos t + e^t \Rightarrow$ \boxed{(f*g)(t)=\frac{\sin t-\cos t + e^t}{2}}
