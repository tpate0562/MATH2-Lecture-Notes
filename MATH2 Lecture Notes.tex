...
Defined as: $\mathcal{U}(t-a)\begin{cases}
    0 & 0 \leq t < a
    \\ 1 & t \geq a
\end{cases}$, the $t$ value at which the argument is equal to 0 is where the jump happens
\\$\mathcal{U}(t-a)$: \begin{tikzpicture}[>=stealth,scale=1.0, baseline={(current bounding box.center)}]
    ...
    \draw[fill=white,draw=red,thick] (2,0) circle (2pt);
    \draw[fill=red,draw=red] (2,1) circle (2pt);
    \node[below] at (2,0) {\(a\)};
    \node[left]  at (0,1) {\(1\)};
...
\\$k\mathcal{U}(t-a)$: \begin{tikzpicture}[>=stealth,scale=1.0, baseline={(current bounding box.center)}]
    ...
    \draw[fill=white,draw=orange,thick] (2,0) circle (2pt);
